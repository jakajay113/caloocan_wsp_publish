    ...
    Public Sub _pSubSelectUIPW()
        ...
        Try
            ...
            _nQuery = _
                ...
                "Where SetupGlobalConnectionsDatabases='RPTAS_T' "
            '----------------------------------
            _mQuery = _nQuery & _nWhere
...
"   FROM [RPTASTEMP].[dbo]." & _mUseTableTmpBill & "_tmp_Billing_TOP_HORZTAL  a  Order by origtdn,part   "
    ...
    Public Sub _pSubInsertBrHlp()
        Try
            ...
            _nQuery = _
             ...
             ") " & _
             " select distinct REGION,PROV,CITY,DIST_NO,TDN,'" & Format(Now, "MM/dd/yyyy") & "' from rptmast where tdn in (" & _mTDN & ")" & _
             " "
    ...
    Public Sub _pSubGetYear()
        Try
            ...
            '----------------------------------
            _nQuery = _
                    "with dt as (select year(GETDATE()) as Yr union all select yr+1 from dt where yr < year(GETDATE())+2) select yr,datepart(q,getdate())QTR,year(getdate())CURYR from dt"
            '    _mRegion_mProv_mCity_mDistrict
            '----------------------------------
            _mQuery = _nQuery
